...
class DataBaseService {
  ...
  Future<QuerySnapshot> _addOrder(OrderModel pedido) async {
    await _firestoreDB
        .collection('pedidos')
        .document(pedido.uid)
...
/////////////////////////////pedidos/////////////////////////
  ...
  Future<List<OrderModel>> getFormulasSolicitadas() async {
    final List<OrderModel> myList = new List<OrderModel>();
    final _docReference = await _firestoreDB
        .collection('pedidos')
        .where('state', isEqualTo: 'solicitado')
        .getDocuments();
    for (var item in _docReference.documents) {
      myList.add(new OrderModel(
        uid: item.documentID,
  ...
  Future<List<AceptadosModel>> getPedidosAceptados() async {
    final List<AceptadosModel> myList = new List<AceptadosModel>();
    final _docReference = await _firestoreDB
        .collection('pedidos_accept')
        .where('state', isEqualTo: 'Aceptado')
        .getDocuments();
    for (var item in _docReference.documents) {
      myList.add(new AceptadosModel(
        uid: item.documentID,
        uidPac: item.data['uidPac'],
        uidMed: item.data['uidMed'],
        applicationDate: item.data['applicationDate'],
  ...
  Future setOrder(OrderModel item) async {
    return await _firestoreDB.collection('pedidos').document(item.uid).setData({
      'uidMed': item.uidMed,
      'uidPac': item.uidPac,
